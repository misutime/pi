...
| 取消 | `$/cancelRequest` 通知 + `peer.cancelRequest()` |
...
SubAgentMethods = {
  ...
  Cancel:   "$/cancelRequest",  // 父 → Worker：取消任务（notification）
...
5. 监听 abort signal → 先发 `$/cancelRequest` 通知（优雅），3s 后 `SIGTERM` → `SIGKILL`
...
独立进程，启动即创建 `WorkerIpcTransport` + `JsonRpcPeer`，注册 `agent/run` request handler 和 `$/cancelRequest` notification handler。
...
| 优雅终止 | abort 先发 `$/cancelRequest` 通知，3s grace 后才 SIGTERM → SIGKILL |
